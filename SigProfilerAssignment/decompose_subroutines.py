...
def getProcessAvg(
    ...
    if signature_database != None:  # pd.core.frame.DataFrame:
        ...
        if samples.shape[0] == signature_database.shape[0]:
            ...
            sys.exit(
                "The Signatures and the custom signature database have different context types."
            )
...
def signature_decomposition(
    ...
    lognote = open(
        directory
        + "/Solution_Stats/Cosmic_"
        + mutation_context
        + "_Decomposition_Log.txt",
        "w",
    )
    lognote.write(
    ...
    newsigmatrixidx = (
        list()
    )  # will create the original id of newsignature to help to record the original matrix
    ...
    for i, j in zip(range(signatures.shape[1]), denovo_signature_names):
        ...
        L1dist = np.linalg.norm(
            signatures[:, i] - np.dot(sigDatabase, exposures), ord=1
        ) / np.linalg.norm(signatures[:, i], ord=1)
        exposure_percentages = (
            exposures[np.nonzero(exposures)]
            / np.sum(exposures[np.nonzero(exposures)])
            * 100
        )
        ...
        if (
            ...
            byte_plot = sp.run_PlotDecomposition(
                originalProcessAvg[denovo_cols],
                denovo_name,
                cosmic_sigs_DF[basis_cols],
                basis_names,
                weights,
        ...
        strings = (
            "Signature %s-%s,"
            + " Signature %s (%0.2f%s) &" * (len(np.nonzero(exposures)[0]) - 1)
            + " Signature %s (%0.2f%s), %0.2f,  %0.2f, %0.3f, %0.2f, %0.2f\n"
        )
        # new_signature_thresh_hold = 0.8
        if (
            similarity > new_signature_thresh_hold
            and cosine_similarity_with_four_signatures > new_signature_thresh_hold
        ):  ########### minimum signtatures and cosine similarity needs to be fitted to become a unique signature
    ...
    try:
        ...
        print(
            "The context-"
            + str(mtype_par)
            + " decomposition pages were not able to be merged."
        )
    ...
    return {
        "globalsigids": list(detected_signatures),
        "newsigids": newsig,
        "globalsigs": globalsigmats,
        "newsigs": newsigsmats / 5000,
...
def custom_signatures_plot(signatures, output):
    with PdfPages(output + "/Custom_Signature_Plots.pdf") as pdf:
        ...
        for i in range(1, signatures.shape[1]):
            ...
            plt.figure(figsize=(10, 3))
            plt.bar(
                list(range(1, 1 + len(signatures.iloc[:, i]))), signatures.iloc[:, i]
            )
